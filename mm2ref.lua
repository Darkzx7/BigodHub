-- ══════════════════════════════════════════════════════════════════════════════
-- MM2 v18 — SILENT AIM TYPE-SAFE + SKELETON ESP + SHOOT BUTTON FIXADO
-- ══════════════════════════════════════════════════════════════════════════════
...
local ui = RefLib.new("mm2 v18", "rbxassetid://131165537896572", "ref_mm2v18")
...
-- ══════════════════════════════════════════════════════════════════════════════
-- SILENT AIM — TYPE-SAFE
-- ══════════════════════════════════════════════════════════════════════════════
--
-- O PROBLEMA ANTERIOR era que o hook substituía args sem checar o tipo deles.
-- O GunClient do MM2 usa FireServer(bool, Vector3) onde:
--   arg1 = bool (isMoving / isRunning / algum estado do player)
--   arg2 = Vector3 (posição do alvo / mouse)
--
-- REGRA DE OURO do hook TYPE-SAFE:
--   - Só substitui um arg se ele FOR Vector3 ou CFrame
--   - JAMAIS toca em args do tipo bool, number, string ou Instance
--   - Se nenhum arg for Vector3/CFrame, não faz nada (passa original)
--
-- Isso garante que a assinatura original nunca seja quebrada.
-- ══════════════════════════════════════════════════════════════════════════════
...
local function getPredictedPos(targetPlayer)
    ...
    local part = head or hrp; if not part then return nil end

    local now    = tick()
    local curPos = part.Position
    local prev   = _prevTargetPos[targetPlayer]
    local prevT  = _prevTargetTime[targetPlayer]
    local predicted = curPos

    if prev and prevT and (now - prevT) > 0.01 and (now - prevT) < 0.5 then
        local vel = (curPos - prev) / (now - prevT)
        predicted = curPos + vel * 0.1  -- 100ms lookahead
    end
...
-- Verifica se um valor é "posicional" (algo que pode ser substituído pela posição do alvo)
local function isPosArg(v)
    local t = typeof(v)
    return t == "Vector3" or t == "CFrame"
end
...
local function hookSilentAim()
    ...
    mt.__namecall = newcclosure(function(self, ...)
        if self == nil then return old_nc(self, ...) end

        local method = getnamecallmethod()
        if method ~= "FireServer" then return old_nc(self, ...) end

        -- É um RemoteEvent?
        local ok, isRE = pcall(function() return self:IsA("RemoteEvent") end)
        if not (ok and isRE) then return old_nc(self, ...) end

        -- Silent aim ativo?
        if not silentAimOn then return old_nc(self, ...) end

        -- É um remote de uma gun tool? (pai direto ou 2 níveis acima)
        local isGunRemote = false
        local par = self.Parent
        if par then
            local function checkTool(obj)
                if not obj then return false end
                local ok2, isTool = pcall(function() return obj:IsA("Tool") end)
                return ok2 and isTool and GUN_NAMES[obj.Name] == true
            end
            isGunRemote = checkTool(par) or checkTool(par.Parent)
        end
        if not isGunRemote then return old_nc(self, ...) end

        -- Temos um alvo?
        local target  = getSilentTarget()
        local hitPos  = target and getPredictedPos(target)
        if not hitPos then return old_nc(self, ...) end

        -- ── TYPE-SAFE SUBSTITUTION ────────────────────────────────────────
        -- Percorre os args e substitui SOMENTE os que são Vector3 ou CFrame.
        -- Todos os outros (bool, number, string, Instance, nil) ficam intactos.
        local args    = { ... }
        local replaced = false

        for i, v in ipairs(args) do
            if isPosArg(v) then
                -- Mantém o mesmo tipo do original
                ...
                replaced = true
                -- Só substituímos o PRIMEIRO arg posicional (é o alvo)
                -- Se houver mais posições depois (ex: origin), não mexemos
                break
            end
        end

        -- Se não havia nenhum arg posicional, deixa passar sem modificar
        -- (provavelmente não é um tiro, é outro FireServer da gun)
        if not replaced then return old_nc(self, ...) end

        return old_nc(self, table.unpack(args))
    end)
end

-- Hook no UserInputService: engana o GunClient pra usar caminho de PC no mobile
local _uisHooked = false
local function hookUIS()
    if _uisHooked then return end
    _uisHooked = true
...
-- Dispara tiro com silent aim ativado
-- Garante que a gun está equipada e chama Activate()
-- O hook __namecall redireciona o FireServer resultante
...
local function fireWithSilentAim()
    ...
    -- Aponta HRP pro alvo (melhora o raycast do servidor)
    local hrp    = myHRP()
    ...
    -- Ativa o tiro — o hook __namecall vai redirecionar o FireServer
    local wasSA = silentAimOn
    silentAimOn = true
    hookSilentAim()
    hookUIS()
    ...
    for _, part in ipairs(chr:GetDescendants()) do
        ...
        for _, myPart in ipairs(myParts) do
            pcall(function()
                ...
                nc.Part0 = part; nc.Part1 = myPart; nc.Parent = part
...
local function restoreAllHitboxes()
    local ps = {}; for p in pairs(hitboxCache) do table.insert(ps, p) end
    for _, p in ipairs(ps) do restoreHitboxOfPlayer(p) end; hitboxCache = {}
end
...
-- ── SKELETON ESP ─────────────────────────────────────────────────────────────
-- Desenha "bones" entre as joints do character usando BoxHandleAdornments.
-- Parece um boneco de palito / wireframe.
-- Cada "bone" = um BoxHandleAdornment fino (0.06 studs de largura) pintado
-- com a cor do papel do jogador, ligando dois BaseParts adjacentes.
-- ─────────────────────────────────────────────────────────────────────────────

local espOn  = false
local espMax = 300
local espCache = {}  -- { [player] = { bones={...}, bb=BillboardGui, hl=Highlight } }

-- Pares de partes que formam o esqueleto (R15 + R6 fallback)
...
local BONE_THICKNESS = Vector3.new(0.06, 0.06, 1)  -- Z = comprimento (será escalado)

-- Cria um BoxHandleAdornment representando um osso entre partA e partB
local function makeBone(partA, partB, color, adorneeRoot)
    local bone = Instance.new("BoxHandleAdornment")
    bone.AlwaysOnTop = true
    bone.ZIndex      = 5
    bone.Color3      = color
    bone.Transparency = 0.15
    bone.Size        = BONE_THICKNESS
    bone.Adornee     = adorneeRoot
    bone.Parent      = adorneeRoot
    return bone
end

-- Atualiza a CFrame e o comprimento de um bone entre dois pontos
local function updateBone(bone, posA, posB)
    if not bone or not bone.Parent then return end
    local mid  = (posA + posB) * 0.5
    local len  = (posA - posB).Magnitude
    if len < 0.01 then bone.Size = Vector3.new(0.06, 0.06, 0.01); return end
    local dir  = (posB - posA).Unit
    local cf   = CFrame.new(mid, mid + dir)
    bone.CFrame = cf
    bone.Size   = Vector3.new(0.06, 0.06, len)
end

local function removeESP(p)
    local d = espCache[p]; if not d then return end
    for _, bone in ipairs(d.bones or {}) do
        pcall(function() if bone and bone.Parent then bone:Destroy() end end)
    end
    pcall(function() if d.bb and d.bb.Parent then d.bb:Destroy() end end)
    espCache[p] = nil
end
...
local function buildESP(p)
    ...
    -- Detecta R15 vs R6
    local pairs_to_use = BONE_PAIRS_R15
    if chr:FindFirstChild("Torso") and not chr:FindFirstChild("UpperTorso") then
        pairs_to_use = BONE_PAIRS_R6
    end

    -- Cria bones
    local bones = {}
    local boneData = {}  -- { bone, nameA, nameB }
    for _, pair in ipairs(pairs_to_use) do
        local pA = chr:FindFirstChild(pair[1])
        local pB = chr:FindFirstChild(pair[2])
        if pA and pB then
            local bone = makeBone(pA, pB, col, hrp)
            table.insert(bones, bone)
            table.insert(boneData, { bone=bone, nameA=pair[1], nameB=pair[2] })
        end
    end

    -- BillboardGui: nome + papel + distância (simples, sem HP)
    local bb = Instance.new("BillboardGui")
    ...
    nm.TextStrokeTransparency=0.1; nm.TextXAlignment=Enum.TextXAlignment.Center
    nm.Text=p.DisplayName; nm.Parent=bb
    ...
    espCache[p] = { bones=bones, boneData=boneData, bb=bb, nm=nm, dl=dl, col=col }
end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        -- Atualiza cor pelo papel
        local role = getRole(p); local col = ROLE_COLOR[role] or ROLE_COLOR.unknown
        if col ~= d.col then
            d.col = col
            d.nm.TextColor3 = col
            for _, bone in ipairs(d.bones) do
                if bone and bone.Parent then bone.Color3 = col end
            end
        end

        -- Atualiza label
        d.nm.Text = p.DisplayName
        d.dl.Text = dist.."m  ["..ROLE_LABEL[role].."]"

        -- Atualiza posição dos bones
        for _, bd in ipairs(d.boneData) do
            local pA = chr:FindFirstChild(bd.nameA)
            local pB = chr:FindFirstChild(bd.nameB)
            if pA and pB and bd.bone and bd.bone.Parent then
                updateBone(bd.bone, pA.Position, pB.Position)
            end
        end
    end
end)
...
local secESP = tabESP:Section("player esp (skeleton)")
local t_esp = secESP:Toggle("skeleton esp (boneco de palito)", false, function(v)
    espOn = v
    if not v then
...
local function makeItemBB(obj, adornee)
    ...
    local conn; conn=RunService.RenderStepped:Connect(function()
        ...
        local hrp=myHRP()
        if hrp and adornee and adornee.Parent then dl.Text=math.floor((hrp.Position-adornee.Position).Magnitude).."m" end
    end)
...
local function scanItems()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if GUNDROP_NAMES[obj.Name] and obj:IsA("BasePart") then task.spawn(makeItemBB, obj, obj) end
    end
end
...
secItemEsp:Button("tp to gun", function()
    ...
    if best then
        ...
        ui:Toast("rbxassetid://131165537896572","tp gun","nenhuma gun dropada",ROLE_COLOR.unknown) end
end)
...
-- ── SILENT AIM ──────────────────────────────────────────────────────────────
secSheriff:Divider("silent aim (type-safe)")
local t_sa = secSheriff:Toggle("silent aim (PC + Mobile)", false, function(v)
    ...
    if v then
        ...
        ui:Toast("rbxassetid://131165537896572","[Silent Aim]",
            "ativo — atire normalmente, mira vai pro murderer",ROLE_COLOR.sheriff)
    else
...
-- ── SHOOT BUTTON ─────────────────────────────────────────────────────────────
-- Usa fireWithSilentAim() que:
--   1. Força silentAimOn = true temporariamente
--   2. Ativa hookSilentAim() (type-safe)
--   3. Chama gunTool:Activate()
--   4. O hook intercepta o FireServer e substitui a posição pelo alvo
...
local function buildShootBtn()
    ...
    card.Size=UDim2.new(0,110,0,56); card.Position=UDim2.new(1,-120,1,-220)
    ...
    local function setBtnState(text, color)
        if not btn.Parent then return end; btn.Text=text; btn.BackgroundColor3=color
    end
    btn.Activated:Connect(function()
        if busy then return end
        local myRole = getRole()
        if myRole ~= "sheriff" and myRole ~= "hero" then
            setBtnState("SEM GUN", T.err); task.delay(1.2, function() setBtnState("ATIRAR", ROLE_COLOR.sheriff) end); return
        end
        if not getSilentTarget() then
            setBtnState("SEM ALVO", T.warn); task.delay(1.2, function() setBtnState("ATIRAR", ROLE_COLOR.sheriff) end); return
        end
        busy = true; setBtnState("...", Color3.fromRGB(80,80,80))
        local ok = fireWithSilentAim()
        task.wait(0.15)
        setBtnState(ok and "FIRED!" or "MISS", ok and T.ok or T.err)
        task.wait(0.8); setBtnState("ATIRAR", ROLE_COLOR.sheriff); busy = false
    end)
end
...
local gunAuraOn=false; local lastGunAura=0; local gunAuraCD=0.8; local gunAuraDist=18
local function gunAuraLoop()
    while gunAuraOn do
        ...
        if (hrp.Position-mHRP.Position).Magnitude>gunAuraDist then
            hrp.CFrame=mHRP.CFrame*CFrame.new(0,0,-gunAuraDist*0.6)
            task.wait(0.08); hrp=myHRP(); if not hrp then continue end
        end
        hrp.CFrame=CFrame.lookAt(hrp.Position,mHRP.Position)
        task.wait(0.04); lastGunAura=tick()
        fireWithSilentAim()
    end
end
...
local s_gacd=secSheriff:Slider("gun aura cooldown (x0.1s)", 2, 30, 8, function(v) gunAuraCD=v/10 end)
...
local knifeAura=false; local knifeRange=12; local knifeCd=0.35; local _knifeConn=nil
local function knifeAuraLoop()
    if _knifeConn then _knifeConn:Disconnect(); _knifeConn=nil end
    task.spawn(function()
        local bp=player:FindFirstChild("Backpack"); if not bp then return end
        local hum=player.Character and player.Character:FindFirstChildOfClass("Humanoid"); if not hum then return end
        for name in pairs(KNIFE_NAMES) do
            local t=bp:FindFirstChild(name); if t then hum:EquipTool(t); break end
        end
    end)
    local accum=0; local cachedRole="unknown"; local roleTimer=0; local playerHRPs={}
    _knifeConn=RunService.Heartbeat:Connect(function(dt)
        if not knifeAura then _knifeConn:Disconnect(); _knifeConn=nil; return end
        ...
        if best then
            ...
            if math.abs(hrp.CFrame.LookVector:Dot(lookCF.LookVector)-1)>0.01 then hrp.CFrame=lookCF end
            pcall(function() knife:Activate() end)
        end
    end)
end
...
secMurd:Button("matar mais proximo", function()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        ...
        if ph and isAlive(p) then
            local d=(hrp.Position-ph.Position).Magnitude; if d<bestD then best=p; bestD=d end
        end
    end
...
local function surviveLoop()
    while survOn do
        ...
        local hrp=myHRP(); if not mh or not hrp then continue end
...
-- CONFIG
-- ══════════════════════════════════════════════════════════════════════════════
ui:BuildConfigTab(tabCfg, "ref_mm2v18")

task.delay(0.9, function()
    local role=getRole()
    ui:Toast("rbxassetid://131165537896572",
        "mm2 v18  ["..ROLE_LABEL[role].."]",
